...
static void get_curr_duration(int seconds) {
    if(strcmp(curr_duration, total_duration) != 0) {
        format_time(seconds, curr_duration);
    }

    //LOGI(program_name, "vf = %s\n", vfilters_list[nb_vfilters - 1]);
}


static void get_total_duration(VideoState *is) {

    int tns  = is->ic->duration / 1000000LL;
    format_time(tns, total_duration);
}
...
static void video_display(VideoState *is) {
    ...
    // 开始绘制文本
    draw_text(renderer, curr_duration, 10, screen_height - 200);
    draw_text(renderer, total_duration, screen_width - 200, screen_height - 200);
    //刷新显示视频
...
display:
    ...
    if(show_status) {
        ...
        if(!last_time || (cur_time - last_time) >= 30000) {
            ...
            // 当前播放的时间
            get_curr_duration((int)get_master_clock(is));
...
static void event_loop(VideoState *cur_stream) {
    ...
    Uint32 curr_timestamp = 0, last_timestamp = 0;
    for(;;) {
        double x = 0.0f;
        ...
        case SDL_FINGERDOWN: // 触摸按下
            ...
            LOGI(program_name, "x = %f\n", touch_x);
            LOGI(program_name, "y = %f\n", touch_y);
            LOGI(program_name, "screen width = %d\n", screen_width);
            LOGI(program_name, "screen height = %d\n", screen_height);
...
int main(int argc, char **argv) {
    ...
    // 初始化字体文件
    init_font("/system/fonts/DroidSans.ttf", 45);
